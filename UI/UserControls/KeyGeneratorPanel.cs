...
namespace CryptoEdu.UI.UserControls
{
    public class KeyGeneratorPanel : UserControl
    {
        ...
        private Control BuildPasswordSection()
        {
            ...
            var numLen = new NumericUpDown { Minimum = 6, Maximum = 128, Value = 20, Width = 76, Font = AppTheme.FontBody, Margin = new Padding(0, 3, 14, 0), BackColor = AppTheme.InputBg, ForeColor = AppTheme.TextPrimary };
            ...
            var txtPw = new TextBox { Dock = DockStyle.Fill, Font = new Font("Cascadia Code", 16, FontStyle.Bold), BorderStyle = BorderStyle.None, ReadOnly = true, TextAlign = HorizontalAlignment.Center, ForeColor = AppTheme.Accent, BackColor = AppTheme.OutputBg };
            var btnCopy = ControlFactory.Pill("⎘ Copy", AppTheme.ButtonMutedBg, AppTheme.ButtonMutedFg, 90);
            var outRow  = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 1, BackColor = AppTheme.OutputBg, Padding = new Padding(10) };
            ...
            tbl.Controls.Add(ControlFactory.PageTitle("Password Generator"), 0, 0);
            tbl.Controls.Add(ControlFactory.SubTitle("Cryptographically-secure passwords via System.Security.Cryptography"),   0, 1);
        ...
        private Control BuildRsaSection()
        {
            ...
            hdrRow.Controls.Add(ControlFactory.PageTitle("RSA 2048-bit Key Pair"), 0, 0);
            ...
            var btnCpPub  = ControlFactory.Pill("⎘ Copy Public",  AppTheme.ButtonMutedBg, AppTheme.AccentSuccess, 120);
            var btnCpPriv = ControlFactory.Pill("⎘ Copy Private", AppTheme.ButtonMutedBg, AppTheme.AccentDanger, 120);
            ...
            boxRow.Controls.Add(WrapBox(txtPub), 0, 0);
            boxRow.Controls.Add(WrapBox(txtPriv), 1, 0);

            tbl.Controls.Add(hdrRow,  0, 0);
            tbl.Controls.Add(ControlFactory.SubTitle("Asymmetric keys for RSA encryption operations"),  0, 1);
        ...
        private static Panel WrapBox(Control inner)
        {
            var p = new Panel { BackColor = AppTheme.InputWrapBg, Dock = DockStyle.Fill, Padding = new Padding(6), Margin = new Padding(0, 0, 4, 0) };
